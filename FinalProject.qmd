...
Our first data visualization is a scatterplot that explores the relationship of child mortality and daily income over time. Each point on the plot represents a country's child mortality rate and daily income for an individual year. We do not see an exactly linear relationship, rather a slighly parabolic curve as child mortality rate decreases as daily income increases. There are few real outliers in this data, as most points are packed very tightly together.
```{r}
data |>
  ...
  labs(title = "Child Mortality vs. Daily Income",
       ...
       y = "Child Mortality")
```
While the above graph shows us the relationship of daily income and child mortality over all years, we are curious to see how the relationship between daily income and child mortality has changed over time. Below, is an animated scatterplot that displays the relationship between child mortality and daily income by year. We see that as more time passes, child mortality rate drops, and daily income starts to move higher up. We see that daily income increases the most after 1960, while child mortality continues to get closer to 0.
```{r}
time<- data |>
...
mod <- lm(avg_mortality~avg_income, data=mod_data)
summary(mod)
```
